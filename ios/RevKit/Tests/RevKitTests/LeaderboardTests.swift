...
struct LeaderboardTests {
    ...
    @Test func rankByEmpireScore() {
        ...
        // give the local player the highest value-weighted score so it sorts to the top
        ...
        // descending by value-weighted empire score
        let scores = board.map(\.empireScore)
        #expect(scores == scores.sorted(by: >))

        // local player on top with 3 plain tiles and flagged
